<!--breadcrumbs-->
  <div id="content-header">
    <div id="breadcrumb"> 
    	...
    	<a href="<?php echo $base_url;?>/?p=m_jk" title="Data Jenis Kendaraan" class="tip-bottom"><i class="icon-user"></i> Jenis Kendaraan</a>    	
    </div>
  </div>
<!--End-breadcrumbs-->
...
<div class="container-fluid">
	...
	<div class="row-fluid">
		<div class="span12">
			...
			<a href="javascript:void(0)" class="btn btn-primary" onclick="showAddForm()"  id="add_form"><i class="icon-plus"></i> Tambah Jenis Kendaraan</a>
			...
			<a href="javascript:void(0)" class="btn btn-danger" id="deletekendaraan"><i class="icon-trash"></i> Hapus Jenis Kendaraan</a>
			<?php } ?>
		</div>
	</div>
				...
				<div class="widget-content nopadding">
					<form class="form-horizontal" id="form-add">
				        <div class="control-group">
				          ...
				          <div class="controls">
				            <input type="text" name="jenis_kendaraan" class="span5" required>
				          </div>
				        </div>
...
<div id="edit-modal" class="modal hide">
  <div class="modal-header">
    ...
    <h3>Ubah Data Jenis Kendaraan</h3>
  </div>
    			...
    			<div class="widget-content nopadding">
    				<form class="form-horizontal" id="form-edit">
    			        ...
    			        <div class="control-group">
    			          <label class="control-label">Jenis Kendaraan</label>
    			          <div class="controls">
    			            <input type="text" required id="edit_jenis_kendaraan">
    			          </div>
    			        </div>

    			        <input type="hidden" value="" id="id_jenis">
...
<script type="text/javascript">
	...
	$(document).ready(function () {
	    ...
	    tbl_kendaraan= $('#tbl_kendaraan').DataTable( {
	        ...
	        "ajax" : "jenis_kendaraan_process.php?action=get",
	        select: {
	            style: 'multi+shift',
	        },
			columns:[
	    ...
	    $('#deletekendaraan').click( function(){
	            ...
	            if(count > 0){ /* if has selected count >= 1 */
	                ...
	                $.messager.confirm('Delete Confirmation', 'Konfirmasi penghapusan ' + count + ' Data Jenis Kendaraan. <br><br><br> <strong class="text-danger">Perhatian : Proses ini akan menghapus seluruh data yang dipilih</strong>', function(r){
	                    if(r){
	                        for (var i = 0; i <= count - 1; i++) {
	                            //console.log(item[i]['id_inkaso']);
	                            var did=item[i]['id_jenis'];
	                            $.post( 'jenis_kendaraan_process.php?action=del&id=' + did).done(function(){tbl_kendaraan.ajax.reload(); });
	                        };
	                    }
	                });
	            }else{
	    ...
	    $('#tbl_kendaraan tbody').on('click', '.edit-btn', function () {
	        $('#edit-modal').modal();
	        $('#id_jenis').val(tbl_kendaraan.row($(this).parents('tr')).data().id_jenis);
	        $('#edit_jenis_kendaraan').val(tbl_kendaraan.row($(this).parents('tr')).data().nama_jenis);
	    }); 
	});
	...
	function saveJenisKendaraan(){
	    $.ajax({
	        url: 'jenis_kendaraan_process.php',
	        type: 'POST',
	        data: {
	            jenis: $('[name=jenis_kendaraan]').val(),
	            action: $('[name=action]').val(),
	            id_jenis: $('[name=id_jenis]').val(),
	        },
